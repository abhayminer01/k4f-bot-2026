...
module.exports = {
  ...
  run: async ({ client, interaction }) => {
    ...
    try {
      ...
      if (!role) {
        return interaction.editReply({ content: '❌ Role not found. Check WL_PENDING_ROLE_ID' });
      }

      if (!logChannel || !logChannel.isTextBased()) {
        return interaction.editReply({ content: '❌ Log channel not found or not a text channel.' });
      }
      ...
      // 3️⃣ Reply to staff (edit deferred reply)
